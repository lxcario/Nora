...
export async function getCompletionSuggestion(
  ...
  // Auth + rate limit check - LLM calls must never be reachable by
  // unauthenticated clients (cost abuse + prompt-injection surface).
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) {
    return { error: "Sign in to get a suggestion." };
  }
